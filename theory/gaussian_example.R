# Context:
#  Suppose X is generated by letting each entry be
#   the absolute value of a standard normal.
#  Then if y = X*beta + error for beta > 0,
#   and we let beta_hat = argmin ||X*beta_h - y||^2 st beta_h > 0
#   then yh = X*beta_hat is quite close to mu=X*beta
#   even for p much larger than n.
#  This no longer holds if entries of X are generated from standard normal
#   or if beta is allowed to have negative entries.
...
# conclusion: given n < O(exp(epsilon * p))
#  the positive cone of {x_1,...,x_n}
#  is well approximated by the circular cone with central axis pointing in {1,1,...,1}
#  and angle 2*arctan(sqrt(pi/2-1)) at the origin
